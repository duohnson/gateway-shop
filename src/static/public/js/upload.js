...
async function handleUploadSubmit(e) {
    ...
    const cantidad = parseInt(form.precio.value);
    const marca = form.brand.value.trim();
    const color = form.color.value.trim();
    const imgFile = form.img.files[0];

    // VALIDATE all fields are filled
    if (!nombre || isNaN(precio) || !imgFile || !descripcion || !categoria) {
    ...
    const brand = marca.toString()
    const image = imgUrl.toString()
    ...
    try {
        ...
        if (res.ok) {
            ...
            productos.push({ id: Date.now(), nombre, precio, descripcion, categoria, img: imgUrl });
            localStorage.setItem('productos', JSON.stringify(productos));
